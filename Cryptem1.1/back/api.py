...
def register_api_routes(app_server, bot_instance):
    ...
    def get_signals_from_csv():
        """
        Lê os sinais do arquivo CSV e os formata para o frontend.
        """
        signals_list = []
        csv_file_path = os.path.join(os.path.dirname(__file__), 'sinais_lista.csv')
        app_server.logger.debug(f"Attempting to read CSV from: {csv_file_path}")
        try:
            with open(csv_file_path, mode='r', encoding='utf-8') as file:
                ...
                for row in csv_reader:
                    ...
                    entry_price = 0.0
                    target_price = 0.0
                    change_percentage_str = "N/A"
                    # NEW: Initialize quality_score before the try block
                    quality_score = 0.0 

                    # Calcular changePercentage
                    try:
                        entry_price = float(row.get('entry_price', 0))
                        target_price = float(row.get('target_price', 0))
                        if entry_price != 0:
                            change_percentage = ((target_price - entry_price) / entry_price) * 100
                            change_percentage_str = f"{change_percentage:.2f}%"
                    except (ValueError, TypeError):
                        app_server.logger.warning(f"Could not calculate changePercentage for row {row_count}")
                    ...
                    # Mapear tipo de sinal (manter LONG/SHORT para a interface Signal)
                    signal_type = row.get('type', '').upper()
                    
                    # Create signal object with all required fields
                    signal_obj = {
                        "symbol": row.get('symbol', ''),
                        "type": signal_type, # Envia LONG/SHORT
                        "entry_price": entry_price, # Envia como float, snake_case
                        "entry_time": row.get('entry_time', ''),
                        "target_price": target_price, # Envia como float, snake_case
                        "status": row.get('status', ''),
                        "quality_score": quality_score, # Now guaranteed to be defined
                        "signal_class": "Sinais Premium" if is_premium else "Sinais Básicos"
                    }

                    # Ensure no undefined values
                    for key, value in signal_obj.items():  # Atualizado para usar signal_obj
                        if value is None:
                            signal_obj[key] = ''

                    app_server.logger.debug(f"DEBUG: signal_obj antes de append: {signal_obj}")
                    signals_list.append(signal_obj)  # Atualizado para usar signal_obj
