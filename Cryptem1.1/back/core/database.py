...
class Database:
    def __init__(self):
        ...
        self.files_to_check = { # Movido para ser um atributo da instância
            self.signals_list_file: ['symbol', 'type', 'entry_price', 'entry_time', 'target_price', 'target_exit_time', 'status', 'exit_price', 'variation', 'result', 'quality_score', 'signal_class', 'trend_score', 'alignment_score', 'market_score', 'strategy_info', 'trend_timeframe', 'entry_timeframe'],
            self.signals_history_file: ['symbol', 'type', 'entry_price', 'entry_time', 'tp3', 'target_exit_time', 'status', 'exit_price', 'variation', 'result', 'score'], # Colunas do history
            ...
            try:
                ...
                if not df.empty:
                    ...
                    for index, row in df.iterrows():
                        # Usar .item() para extrair o valor booleano escalar para o Pyright
                        key = str(row['key']) if pd.notna(row['key']).item() else None
                        value = str(row['value']) if pd.notna(row['value']).item() else None
                        if key is not None:
